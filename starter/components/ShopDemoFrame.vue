...
function startDrag(event: PointerEvent) {
  ...
  if (props.scenario === 'missing-pointer-capture' && !captureFixed.value) return
...
function toggleCapture() {
  captureFixed.value = !captureFixed.value
  position.value = { x: 50, y: 45 }
}
function movePrint(event: PointerEvent) {
  if (!(event.currentTarget instanceof HTMLElement) || !event.currentTarget.hasPointerCapture(event.pointerId) || event.buttons !== 1 || !preview.value) return
    ...
    <div class="stage">
      ...
      <div v-else class="shop">
        ...
        <main v-if="isCustomizer" class="customizer-page">
          ...
          <div ref="preview" class="preview" aria-label="T-shirt print preview" :class="{ narrow }">
            <div class="tee">T</div>
            <button class="mascot" aria-label="Move mascot print" :style="{ left: `${position.x}%`, top: `${position.y}%` }" @pointerdown="startDrag" @pointermove="movePrint" @pointerup="endDrag" @pointercancel="endDrag"><img src="/shop/plush-card-actual.png" alt="" draggable="false" /></button>
          </div>
          <div class="customizer-actions">
            <button v-if="scenario === 'missing-pointer-capture'" :aria-pressed="captureFixed" @click="toggleCapture">{{ captureFixed ? 'Defekt aktivieren' : 'Pointer Capture reparieren' }}</button>
            <button v-else @click="resizeAndMove">Resize preview and drag to 75%, 60%</button>
            <output aria-label="Print position">Position: {{ position.x }}%, {{ position.y }}%</output>
          </div>
        </main>
...
/* Keep the embedded shop independent of the surrounding slide typography. */
.shop h2{color:#25211b;margin:0;font-size:32px;line-height:1.05}
.shop nav strong{color:#25211b}
.shop p{margin:0;line-height:1.5}
.shop .eyebrow{margin-bottom:10px}
.shop .muted{margin-top:18px}
.shop .price{margin:18px 0}
.customizer-page{grid-template-columns:minmax(0,.9fr) minmax(0,1.1fr);grid-template-rows:minmax(0,1fr) auto;gap:14px 28px;padding:20px 42px}
.preview{height:100%;min-height:0;max-height:330px}
.tee{inset:20px 65px}
.customizer-actions{grid-column:1 / -1;min-width:0}
.customizer-actions output{white-space:nowrap}
.mascot{cursor:grab}
